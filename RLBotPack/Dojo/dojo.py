...
class Dojo(BaseScript):
    ...
    def __init__(self):
        ...
        self.race = None
        self.free_goal_mode = False
        self.rule_zero_mode = False
        self.num_trials = 100
    ...
    def run(self):
        while True:
            ...
            self.ticks += 1
            self.paused = False
            # self.paused = packet.game_info.paused
            ...
            if self.ticks == 1:
                ...
                for option in (100, 50, 25, 10):
                    self.free_goal_mode_menu.add_element(UIElement(str(option), function=self.set_free_goal_mode, function_args=option))
    ...
    def race_mode(self, packet):
        ...
        match self.game_phase:
            ...
            case RacePhase.ACTIVE:
                # Check if the current ball location has moved by 3 or more units, which would set up the next race
                if abs(self.game_state.ball.physics.location.x - packet.game_ball.physics.location.x) > 2 \
                    ...
                    if self.human_score >= self.num_trials:
            ...
            case RacePhase.FINISHED:
                ...
                if self.human_score >= self.num_trials:
                    record = records.RaceRecord(
                        number_of_trials=self.num_trials, 
    ...
    def scenario_mode(self, packet):
        if self.paused:
            return
        match self.game_phase:
            case ScenarioPhase.INIT:
                # Set a random seed for reproducible scenarios, log the start time
                np.random.seed(0)
                self.started_time = self.cur_time
                self.game_phase = ScenarioPhase.SETUP

                if self.free_goal_mode:
                    self.timeout = 7.0
                    # Set rule_zero mode to False
                    self.rule_zero_mode = False

            # This is where we set up the scenario and set the game state
            case ScenarioPhase.SETUP:
                if self.free_goal_mode:
                    # Force defensive mode to RECOVERING, randomize the offensive mode
                    self.defensive_mode = DefensiveMode.RECOVERING
                    valid_offensive_modes = []
                    valid_offensive_modes.append(OffensiveMode.BACKPASS)
                    valid_offensive_modes.append(OffensiveMode.CARRY)
                    valid_offensive_modes.append(OffensiveMode.SIDEWALL_BREAKOUT)
                    valid_offensive_modes.append(OffensiveMode.PASS)
                    valid_offensive_modes.append(OffensiveMode.BACKWALL_BOUNCE)
                    valid_offensive_modes.append(OffensiveMode.CORNER)

                    self.offensive_mode = np.random.choice(valid_offensive_modes)
            ...
            case ScenarioPhase.ACTIVE:
                if self.disable_goal_reset == True:
                    # If ball is mostly in the net, count it before it actually goes in
                    # Or else it sends an annoying notification on the screen
                    if packet.game_ball.physics.location.y < utils.BACK_WALL - 30:
                        ...
                        self.game_phase = ScenarioPhase.SETUP
                        return
                    elif packet.game_ball.physics.location.y > (-utils.BACK_WALL + 30):
                        if self.mirrored:
                            self.bot_score += 1
                        else:
                            self.human_score += 1
                ...
                # If rule zero mode is active, timeout if the ball has touched the ground
                if (self.cur_time - self.prev_time) > self.timeout:
                    if packet.game_ball.physics.location.z < 100 or not self.rule_zero_mode:
    ...
    def do_rendering(self):
        ...
        text = f"Welcome to the Dojo. Press 'm' to enter menu"
        ...
        if self.gym_mode == GymMode.SCENARIO:
            ...
            total_score = f"Out of: {self.num_trials}"
            time_since_start = f"Time: {minutes_since_start}:{seconds_str}"
    ...
    def set_scenario_mode(self):
        print(f"Setting scenario mode")
        self.gym_mode = GymMode.SCENARIO
        self.game_phase = ScenarioPhase.INIT

    def set_free_goal_mode(self, trials):
        print(f"Setting free goal mode")
        self.free_goal_mode = True
        self.mirrored = True
        self.gym_mode = GymMode.SCENARIO
        self.game_phase = ScenarioPhase.INIT
        self.num_trials = trials
